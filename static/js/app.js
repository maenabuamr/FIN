...
const App = (() => {
    ...
    function setActiveNav(key) {
        ...
        const idx = ['dashboard','trial_balance','statements','notes','compare'].indexOf(key);
        if (idx >= 0 && items[idx]) items[idx].classList.add('active');
    }
    function renderTopbar() {
        const titles = { dashboard: 'لوحة التحكم', trial_balance: 'ميزان المراجعة', statements: 'القوائم المالية', notes: 'الإيضاحات', compare: 'المقارنات' };
        const tb = $('.topbar');
        if (!tb) return;
    ...
    function goBack() {
        ...
        if (state.currentView === 'statements' || state.currentView === 'notes') return switchView('trial_balance');
        return switchView('dashboard');
    }
    function switchCompany() {
        state.currentCompany = null; state.currentJob = null; state.accounts = []; state.currentView = 'dashboard';
    ...
    async function renderDashboard() {
        ...
        main.appendChild(el('div', { class: 'section', style: 'background:#f9fafb;padding:24px;border-radius:12px;text-align:center;margin-bottom:20px;' },
            el('div', { style: 'font-size:16px;margin-bottom:16px;color:#374151;' }, 'ابدأ'),
        ...
        if (draftJobs.length > 0) {
            main.appendChild(el('div', { style: 'background:#fffbeb;border:1px solid #fcd34d;border-radius:12px;padding:16px;margin-bottom:20px;' },
                el('div', { style: 'display:flex;align-items:center;gap:8px;margin-bottom:8px;' },
                    ...
                    el('strong', { style: 'color:#92400e;' }, `${draftJobs.length} مسودة تنتظر الحفظ`))));
        }
        if (savedJobs.length > 0) {
            ...
            main.appendChild(el('div', { class: 'empty', style: 'background:#fff;border:2px dashed #d1d5db;border-radius:12px;padding:40px;text-align:center;' },
                ...
                el('div', { class: 'empty-text' }, 'ارفع ملف من القائمة الجانبية')));
        }
    }
    ...
    async function renderTrialBalance() {
        ...
        const tabs = el('div', { class: 'tabs' },
            el('button', { class: 'tab' + (state.currentTab === 'list' ? ' active' : ''), onClick: () => { state.currentTab = 'list'; renderTrialBalance(); } }, 'الرفع'),
            el('button', { class: 'tab' + (state.currentTab === 'detail' ? ' active' : ''), onClick: () => { state.currentTab = 'detail'; renderTrialBalance(); } }, 'التفاصيل'),
            el('button', { class: 'tab' + (state.currentTab === 'validate' ? ' active' : ''), onClick: () => { state.currentTab = 'validate'; renderTrialBalance(); } }, 'التشييك'));
        ...
        else main.appendChild(el('div', { class: 'empty' }, el('div', { class: 'empty-text' }, 'ارفع ملف أو افتح ميزان')));
    }
    async function renderTrialBalanceList() {
        ...
        const uploadSec = el('div', { class: 'section' },
            ...
            el('div', { class: 'upload-zone', id: 'upload-zone',
                ...
                el('div', { class: 'upload-text' }, 'اضغط هنا أو اسحب ملف'),
        ...
        const savedJobs = allJobs.filter(j => j.status === 'ready' || j.status === 'committed' || j.status === 'processed');
        const draftJobs = allJobs.filter(j => j.status === 'uploaded' || j.status === 'draft');
        const listSec = el('div', { class: 'section' }, el('h2', { style: 'margin-top:0;' }, `✅ المحفوظة (${savedJobs.length})`));
        if (savedJobs.length === 0) listSec.appendChild(el('div', { class: 'empty', style: 'padding:24px;text-align:center;color:#6b7280;' }, 'لا توجد موازين محفوظة'));
        else savedJobs.forEach(j => {
            listSec.appendChild(el('div', { class: 'job-row' },
                el('div', { class: 'job-info' },
                    el('div', { class: 'job-name' }, '✅ ' + (j.filename || '—')),
                    el('div', { class: 'job-meta' }, `${j.account_count} حساب`)),
                el('div', { class: 'job-actions' },
                    el('button', { class: 'btn btn-primary', onClick: () => openJob(j.job_id) }, 'فتح'),
                    el('button', { class: 'btn btn-outline', onClick: () => deleteJob(j.job_id) }, '🗑'))));
        });
        ...
        if (draftJobs.length > 0) {
            const draftSec = el('div', { class: 'section', style: 'background:#fffbeb;border:1px solid #fcd34d;border-radius:12px;' },
                el('h2', { style: 'margin-top:0;color:#92400e;' }, `📝 مسودات (${draftJobs.length})`));
            draftJobs.forEach(j => {
                draftSec.appendChild(el('div', { class: 'job-row', style: 'background:#fef3c7;border-radius:8px;margin-bottom:8px;' },
                    el('div', { class: 'job-info' },
                        el('div', { class: 'job-name' }, '📝 ' + (j.filename || '—')),
                        el('div', { class: 'job-meta', style: 'color:#78350f;' }, `${j.account_count} حساب`)),
                    el('div', { class: 'job-actions' },
                        el('button', { class: 'btn btn-primary', onClick: () => openJob(j.job_id) }, 'متابعة'),
                        el('button', { class: 'btn btn-primary', onClick: () => commitJob(j.job_id) }, '💾 حفظ'),
                        el('button', { class: 'btn btn-outline', onClick: () => deleteJob(j.job_id) }, '🗑'))));
            });
    ...
    async function handleFileUpload(file) {
        ...
        try {
            ...
            toast('✅ تم الرفع. راجع ثم احفظ', 'success');
    ...
    async function commitJob(jobId) {
        if (!confirm('حفظ هذا الميزان؟')) return;
        try { await api(`/api/jobs/${jobId}/commit`, { method: 'POST' }); toast('✅ تم الحفظ', 'success'); await renderTrialBalance(); } catch (e) { toast('خطأ: ' + e.message, 'error'); }
    }
    async function deleteJob(jobId) {
        if (!confirm('حذف؟')) return;
        try { await api(`/api/jobs/${jobId}`, { method: 'DELETE' }); toast('تم', 'success'); if (state.currentJob === jobId) { state.currentJob = null; localStorage.removeItem('currentJobId'); } await renderTrialBalance(); } catch (e) { toast('خطأ: ' + e.message, 'error'); }
    }
        ...
        try {
            ...
            main.appendChild(el('div', { class: 'section' },
                el('div', { class: 'section-header' },
                    el('div', {}, el('h2', { style: 'margin:0;' }, isDraft ? '📝 مراجعة وتعديل' : '✅ تفاصيل'), el('div', { style: 'font-size:13px;color:#6b7280;margin-top:4px;' }, job.filename || '')),
                    el('div', { style: 'display:flex;gap:8px;flex-wrap:wrap;' },
                        el('button', { class: 'btn btn-outline', onClick: processJob }, '⚙️ توليد القوائم'),
                        isDraft ? el('button', { class: 'btn btn-primary', onClick: () => commitJob(state.currentJob) }, '💾 حفظ') : el('span', { class: 'tag green' }, '✓ محفوظ'))),
                table));
        } catch (e) { main.innerHTML = `<div class="empty">خطأ: ${e.message}</div>`; }
    }
    async function deleteAccount(idx) {
        if (!confirm('حذف؟')) return;
        try { await api(`/api/jobs/${state.currentJob}/accounts/${idx}`, { method: 'DELETE' }); toast('تم', 'success'); await renderEditor(); } catch (e) { toast('خطأ: ' + e.message, 'error'); }
    }
    ...
    async function processJob() {
        try { await api(`/api/process/${state.currentJob}`, { method: 'POST' }); toast('✅ تم توليد القوائم', 'success'); await renderEditor(); } catch (e) { toast('خطأ: ' + e.message, 'error'); }
    }
    ...
    async function renderStatements() {
        const main = $('#main-content'); main.innerHTML = '';
        if (!state.currentJob) { main.innerHTML = '<div class="empty">افتح ميزان محفوظ</div>'; return; }
        try {
    ...
    async function exportFile(fmt_) {
        try { const r = await api(`/api/export/${fmt_}/${state.currentJob}`); const blob = await r.blob(); const url = URL.createObjectURL(blob); const a = el('a', { href: url, download: `report.${fmt_}` }); document.body.appendChild(a); a.click(); a.remove(); toast('تم', 'success'); } catch (e) { toast('خطأ: ' + e.message, 'error'); }
    }
    async function renderNotes() {
        const main = $('#main-content'); main.innerHTML = '';
        if (!state.currentJob) { main.innerHTML = '<div class="empty">افتح ميزان</div>'; return; }
        try {
            const r = await api(`/api/notes/${state.currentJob}`);
            const notes = r.notes || [];
            const sec = el('div', { class: 'section' }, el('h2', { style: 'margin-top:0;' }, '📝 الإيضاحات'));
    ...
    async function renderCompare() { $('#main-content').innerHTML = '<div class="empty">قارن بين فترتين (قريباً)</div>'; }
    const SUB_CATEGORIES = [
        ...
        ['payables', 'الدائنون'], ['short_term_loans', 'قروض قصيرة الأجل'], ['accruals', 'مصروفات مستحقة'], ['other_current_liabilities', 'التزامات متداولة أخرى'],
        ['long_term_loans', 'قروض طويلة الأجل'], ['share_capital', 'رأس المال'], ['reserves', 'الاحتياطيات'], ['retained_earnings', 'أرباح مبقاة'],
    ...
    async function init() { setupNav(); await showCompanySelector(); }
    return { init, goBack };
})();
